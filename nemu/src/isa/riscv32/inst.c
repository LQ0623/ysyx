...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div_r  , Rt, R(rd) = (src2 == 0)? 0:(word_t)((int32_t)src1 / (int32_t)src2));
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , Rt, R(rd) = (src2 == 0)? 0:(int32_t)src1 % (int32_t)src2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , Rt, R(rd) = (src2 == 0)? 0:src1 % src2);
  ...
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , Rt, R(rd) = (src2 == 0)? 0:src1 / src2);
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , Rt, word_t t = SEXT(BITS(i, 31, 20), 12); R(rd) = src1 | t);
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
