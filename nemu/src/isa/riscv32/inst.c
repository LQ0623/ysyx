...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div_r  , Rt, R(rd) = (int32_t)((int64_t)src1 / (int64_t)src2));
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , Rt, int32_t t = SEXT(BITS(i, 31, 25), 12); R(rd) = (word_t)((int32_t)src1 | t));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
